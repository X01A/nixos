# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  motrix = {
    pname = "motrix";
    version = "1.8.19";
    src = fetchurl {
      url = "https://github.com/agalwood/Motrix/releases/download/v1.8.19/Motrix-1.8.19.AppImage";
      sha256 = "sha256-oSO+VH3bZcjnXjECqZgOmsvlOONbfgOq50qVLvHdKfo=";
    };
  };
  orca-slicer = {
    pname = "orca-slicer";
    version = "1.6.4-beta";
    src = fetchurl {
      url = "https://github.com/SoftFever/OrcaSlicer/releases/download/v1.6.4-beta/OrcaSlicer_V1.6.4-beta_Linux.zip";
      sha256 = "sha256-ALqloYdACGM0PbcYExlYVG3bqCJR2r2EmB1UtVfn2Nc=";
    };
  };
  yesplaymusic = {
    pname = "yesplaymusic";
    version = "0.4.7";
    src = fetchurl {
      url = "https://github.com/qier222/YesPlayMusic/releases/download/v0.4.7/YesPlayMusic-0.4.7.AppImage";
      sha256 = "sha256-y0h2u/k4UCrKyE/2j+vIY+K835peSkuWThcwm37jcE4=";
    };
  };
}
